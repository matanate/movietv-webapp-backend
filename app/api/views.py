...
def get_tmdb_search(request):
    ...
    try:
        ...
        # Handle exceptions
        return Response(
            {"error": "An error occurred during the TMDB search."},
...
class BaseViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request):
        ...
        # Fallback to the parent class list method
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        ...
        if self.action in [
            ...
            "retrieve",
            "destroy",
        ]:  # GET/DELETE actions should be performed by the current user or admin
            permission_classes = [IsAdminUser | IsCurrentUser]
...
class ValidationViewSet(viewsets.ViewSet):
    ...
    def create(self, request):
        ...
        # Handle exceptions
            ...
            error_strings = "\n".join(
                error for _key, error_list in e.detail.items() for error in error_list
            )
            return Response(
                {"error": error_strings},
            ...
            return Response(
                {"error": "An Error occurred while sending validation email."},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class PasswordResetViewSet(viewsets.ViewSet):
    ...
    validation_serializer_class = ValidationSerializer
...
class AuthViewSet(viewsets.ViewSet):
    ...
    def google_login(self, request):
        ...
        try:
            ...
            return Response(
                {"error": "Invalid issuer"}, status=status.HTTP_400_BAD_REQUEST
            )
        except ValueError:
            return Response(
            ...
            return Response(
                {"error": "Something went wrong while verifying the google credential"},
                status=status.HTTP_400_BAD_REQUEST,
            )
